...
## Local commands
- Recommended (local E2E): run blog+oauth via `docker compose` when working with Admin (/admin).
- Homepage: `docker compose build web && docker compose up -d web` (static preview can be a simple local server)
